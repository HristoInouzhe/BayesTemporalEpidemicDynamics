...
//
// SUNDIALS functions for the SIKR models
/* -----------------------------------------------------------------
...
void SIR_CVODES(double results_y[][num_out_cvodes], double results_s[][num_out_cvodes][2 + g_inpset->num_basis_spline], double state_params[2 + g_inpset->num_basis_spline],
                 double degree, double m, double nf, double *knots, double *basis, char *sensitivity_parameters[])
//void SIR_CVODES_synth(double **results_y, double ***results_s, double *state_params,
//                 double degree, double m, double nf, double *knots, double *basis, char *sensitivity_parameters[])
{
  ...
  //static double results_y[NEQ][num_out_cvodes];
  realtype *y_data;
  //double *y_data;

  //realtype pbar[2 + g_inpset->num_basis_spline];
  int is;
  N_Vector *yS;
  booleantype sensi, err_con;
  int sensi_meth;
  //double results_s[NEQ][num_out_cvodes][2 + g_inpset->num_basis_spline];
  realtype *s_data;
  ...
  for(j = 0; j < 2 + g_inpset->num_basis_spline; j++){
  ...
  Ith(y,1) = num_pop - data->p[1];
  Ith(y,2) = data->p[1];
  for(j = 1; j < NICOMP; j++){
    Ith(y, 2 + j) = RCONST(0.0);
  }
  Ith(y, NEQ) = RCONST(0.0);
  ...
  retval = CVodeInit(cvode_mem, f_PREV, T0, y);
...
//  atol[2] = ATOL3;
  ...
  retval = CVodeSetJacFn(cvode_mem, Jac_PREV);
  ...
  if (sensi) {
    ...
    yS = N_VCloneVectorArray(2 + g_inpset->num_basis_spline, y);
    ...
    Ith(yS[1], 2) = RCONST(1.0);
    for(j = 1; j < NICOMP; j++){
      Ith(yS[1], 2 + j) = RCONST(0.0);
    }
    Ith(yS[1], NEQ) = RCONST(0.0);


    for (is = 2; is < 2 + g_inpset->num_basis_spline; is++) N_VConst(ZERO, yS[is]);

    /* Call CVodeSensInit1 to activate forward sensitivity computations
       and allocate internal memory for COVEDS related to sensitivity
       calculations. Computes the right-hand sides of the sensitivity
       ODE, one at a time */
    retval = CVodeSensInit1(cvode_mem, 2 + g_inpset->num_basis_spline, sensi_meth, fS_PREV, yS);
  ...
  for (iout=1, tout=T1; iout <= num_out_cvodes; iout++, tout += TAD) {
...
//    printf("results_y: %12.9e %12.9e %12.9e \n", results_y[0][iout-1], results_y[1][iout-1],results_y[2][iout-1]);
    ...
    if (sensi) {
      ...
      for (jj = 0; jj < 2 + g_inpset->num_basis_spline; jj++) {
...
//  PrintFinalStats(cvode_mem, sensi);
  ...
  if (sensi) {
    N_VDestroyVectorArray(yS, 2 + g_inpset->num_basis_spline);  /* Free yS vector */
  }
...
int f_PREV(realtype t, N_Vector y, N_Vector ydot, void *user_data)
{
  ...
  Ith(ydot, NEQ) = NICOMP * p1 * Ith(y, 1 + NICOMP);
              ...
              void *user_data, N_Vector tmp1, N_Vector tmp2)
{
  ...
  sd[NEQ - 1] = NICOMP * p1 * Ith(yS, 1 + NICOMP);
  ...
  if(iS == 0){
    ...
    sd[NEQ - 1] += NICOMP * Ith(y, 1 + NICOMP);
  } else{
    for (iii = 2; iii < 2 + g_inpset->num_basis_spline; iii++){
      if(iS == iii){
	sd[0] += -beta_t * data->basis[iii - 2] * y1 * y_tot / num_pop;
	sd[1] += beta_t * data->basis[iii - 2] * y1 * y_tot / num_pop;
      }
    }
  }
...
void printODE(FILE *odefp) {
    ...
    double parameters_odes[2 + g_inpset->num_basis_spline];
    double results_y[NEQ][kNumObs];
    double results_s[NEQ][kNumObs][2 + g_inpset->num_basis_spline];

    parameters_odes[0] = t_gamma;
    parameters_odes[1] = t_i0;
    for(i = 2; i < (2 + g_inpset->num_basis_spline); i++){
    ...
    dx = (tfin_spl_bas - X0)/(g_inpset->num_basis_spline - g_inpset->spline_pol_degree);
    for (i = 0; i < m; i++){
        knots[i] = X0 + (i - g_inpset->spline_pol_degree) * dx;
    }

    SIR_CVODES(results_y, results_s, parameters_odes, g_inpset->spline_pol_degree, m, nf, knots, basis, sensitivities);
