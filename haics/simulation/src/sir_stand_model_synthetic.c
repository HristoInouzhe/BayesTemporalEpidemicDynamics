...
double PriorSIKR_stand_Incidence(){
    ...
    logp += log_prior_density(params[4], &prior_settings[2]);
...
void GradientSIKR_stand_Incidence(TypeState *st){
    ...
    // Add the derivative of the log-prior density
    prior_derivative = log_prior_density_derivative(t_beta, &prior_settings[j - 2]);
...
void GradientDescentSIKR_stand_Incidence(){
    ...
    while (!isfinite(logPosterior)){
        if(init_attempts < init_max_attempts){
            ...
            // Initialize t_beta prior setting
            prior_settings[2] =initialize_prior(g_inpset->beta_prior, g_inpset->beta_param_1, g_inpset->beta_param_2);
            ...
            state[curr]->pos[4] = sample_from_prior(&prior_settings[2]);  // beta
